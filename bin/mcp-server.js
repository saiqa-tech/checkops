    ...
    async handleBatchOperations(name, args, checkops) {
        switch (name) {
            case 'checkops_bulk_create_forms': {
                ...
                let forms;
                if (typeof checkops.bulkCreateForms === 'function') {
                    forms = await recordBatchOperation(
                        'bulk_create_forms',
                        args.forms.length,
            ...
            case 'checkops_bulk_create_submissions': {
                ...
                if (typeof checkops.bulkCreateSubmissions === 'function') {
                    ...
                    for (const submissionData of args.submissions) {
                        try {
                            const submission = await checkops.createSubmission({
                                formId: args.formId,
                                submissionData
                            });
                            submissions.results.push(submission);
                        } catch (error) {
                            submissions.errors.push({ submissionData, error: error.message });
                        }
                    }
                }
                return {
            ...
            case 'checkops_bulk_create_questions': {
                ...
                if (typeof checkops.bulkCreateQuestions === 'function') {
                    ...
                    for (const questionData of args.questions) {
                        const question = await checkops.createQuestion(questionData);
